...
export function renderRehab(container, state, app) {
  ...
  container.innerHTML = `
        ...
        ${rehabExercises.map(exercise => `<article class="card flat">
          <div style="display:flex;gap:12px;align-items:flex-start">
            ...
            <div class="grow">
              ...
              <div class="submetric">${escapeHtml(app.t(exercise.target))} · ${escapeHtml(app.t(exercise.prescription))}</div>
              <ul style="padding-left:18px;margin:10px 0 0;color:var(--muted);font-size:12px;line-height:1.6">${exercise.cues.map(cue=>`<li>${escapeHtml(app.t(cue))}</li>`).join('')}</ul>
              <div style="margin-top:8px;color:var(--red);font-size:10px">${escapeHtml(app.t('หยุดเมื่อ:'))} ${escapeHtml(app.t(exercise.stopIf))}</div>
            </div>
          </div>
        </article>`).join('')}
      </div>
    ...
    <section class="section">
      ...
      <div class="grid two">${runningDrills.map(drill=>`<article class="card flat"><strong>${escapeHtml(drill.name)}</strong><div class="submetric">${escapeHtml(app.t(drill.purpose))}</div><div style="margin-top:10px;color:var(--mint);font-size:12px;font-weight:800">${escapeHtml(app.t(drill.prescription))}</div></article>`).join('')}</div>
    </section>
  `;
