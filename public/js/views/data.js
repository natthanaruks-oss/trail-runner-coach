...
export function renderData(container, state, app) {
  ...
  container.innerHTML = `
    ...
    <section class="section"><div class="section-head"><h2>Backup</h2><span>แนะนำหลังบันทึกข้อมูลสำคัญ</span></div>
      <article class="card flat"><div class="button-row"><button class="button primary" data-action="export-backup">Export JSON</button><button class="button secondary" data-action="import-backup">Import JSON</button><input id="backup-file" type="file" accept=".json" hidden></div><div class="submetric">Backup รวม Settings, Check-in, Activities, Pain, Workout, Rehab, Gear, InBody และ Sync metadata</div></article>
    </section>
