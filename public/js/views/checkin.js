...
function renderResult(container, readiness) {
  ...
  container.innerHTML = `<article class="card"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><div><div class="card-title">ผลวันนี้</div><div class="metric">${readiness.score}<small>/100</small></div></div>${statusBadge(readiness.status,label)}</div><div class="submetric">Confidence ${readiness.confidence}% · ${escapeHtml(readiness.flags.join(' · ') || 'ไม่มีสัญญาณเตือนเด่น')}</div></article>`;
}
