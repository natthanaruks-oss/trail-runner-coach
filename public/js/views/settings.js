...
export function renderSettings(container,state,app){
  ...
  container.innerHTML=`
    ...
    <form id="settings-form" class="card"><div class="form-grid">
      <div class="field full"><label>ภาษา / Language</label><select name="language"><option value="th" ${s.language!=='en'?'selected':''}>ไทย</option><option value="en" ${s.language==='en'?'selected':''}>English</option></select></div>
      ...
      ${fieldNumber({name:'sleepTargetHours',label:'เป้าหมายการนอน (ชั่วโมง)',value:s.athlete.sleepTargetHours??7.5,min:6,max:10,step:.1})}
  ...
  container.querySelector('#settings-form').addEventListener('submit',async event=>{event.preventDefault();const d=new FormData(event.currentTarget);const nullable=k=>d.get(k)===''?null:Number(d.get(k));await app.store.saveSettings({language:d.get('language')==='en'?'en':'th',athlete:{age:nullable('age'),sex:d.get('sex')||null,heightCm:nullable('heightCm'),weightKg:nullable('weightKg'),maxHr:nullable('maxHr'),restingHrBaseline:nullable('restingHrBaseline'),sleepTargetHours:nullable('sleepTargetHours')||7.5,flatFeet:d.has('flatFeet'),wideForefoot:d.has('wideForefoot')},preferences:{nonExerciseActivityFactor:Number(d.get('activityFactor'))||1.2},nutrition:{bmrKcal:nullable('bmrKcal'),proteinTargetGPerKg:Number(d.get('proteinTargetGPerKg'))||1.8,waterBaseMlPerKg:Number(d.get('waterBaseMlPerKg'))||30}});app.toast('บันทึกการตั้งค่าแล้ว');app.render();});
}
